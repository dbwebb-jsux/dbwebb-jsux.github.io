...
description: Vi skaffar oss en API nyckel för att kunna använda LLMs via ett API.
---
...
I videon nedan visar Emil hur du får en API-nyckel. Efter ett kort tag bör ni få ett mail med API-nyckeln, spara undan den i `models/auth.js` på samma sätt som nyckeln ni har för Lager API:t. Börja med att gå till [BTH:s självserviceportal](https://bth.topdesk.net/) och följ sedan videon.

<YouTube posterQuality="max" id="suOB5SxGn7A" />
...
async sendMessage(element) {
  ...
  const data = {
    ...
    const response = await fetch("https://bth-ai.azure-api.net/student/openai/deployments/gpt-4o-mini/chat/completions?api-version=2025-01-01-preview", {
